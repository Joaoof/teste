...
test.group("Group", (group) => {
  test("it should create group", async (assert) => {
    ...
    const { body } = await supertest(BASE_URL).post('/groups').set('Authorization', `Bearer ${token} `).send(groupPayload).expect(201)
    ...
    assert.equal(body.group.players[0].id, groupPayload.master) // se o id do jogador é igual o do mestre
  })

  test('it should return 422 when required data is not provided', async(assert) => {
    const { body } = await supertest(BASE_URL).post('/groups').set('Authorization', `Bearer ${token} `).send({}).expect(422)
